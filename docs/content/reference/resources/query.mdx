...
- **linear**: Delay increases by `initialDelay` each retry (1s → 2s → 3s → 4s...)
- **fixed**: Constant delay of `initialDelay` between all retries

### Retry Behavior

- Retries occur on transient errors (rate limits, network timeouts, 5xx errors)
- Non-retryable errors (validation failures, 4xx errors) fail immediately
- `status.retryCount` tracks the number of retry attempts made
- Query phase becomes `error` if all retries are exhausted
