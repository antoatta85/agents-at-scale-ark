...
The final chunk also includes complete query status:

```json
{
  "ark": {
    "query": "789",
    "queryStatus": {                 // Complete Query status (final chunk only)
      "phase": "done",
      "tokenUsage": {...},
...
The `queryStatus` field contains the complete [Query status](/reference/resources/query#status) including token usage and A2A metadata. This is only present in the final chunk.
